...
public class DAOcliente implements clienteDAO{
	...
	public List<Cliente> getClientes(String numeroCliente, String apellido, String nombre, Long tipoDocumento, String numeroDocumento){
		return this.getAll().stream().filter(a -> (numeroCliente == null || a.getNumeroCliente().equals(numeroCliente)))
									 .filter(a -> (apellido == null || apellido.equals("") || a.getPersona().getApellido().toLowerCase().startsWith(apellido.toLowerCase())))
									 .filter(a -> (nombre == null || nombre.equals("") || a.getPersona().getNombre().toLowerCase().startsWith(nombre.toLowerCase())))
